...
parser.add_argument('--number-expstart', type=int, default=1,
                    help='start number of experiments.')
parser.add_argument('--number-exp', type=int, default=96,
...
# 加载训练集、验证集和测试集，并获取位置和速度信息的最大值和最小值。
...
off_diag = np.ones([args.num_residues, args.num_residues]    # 生成一个表示蛋白质残基之间交互关系的矩阵，对角线元素为 0，其他元素为 1。
                   ) - np.eye(args.num_residues)

rel_rec = np.array(encode_onehot(np.where(off_diag)[1]), dtype=np.float32)
rel_send = np.array(encode_onehot(np.where(off_diag)[0]), dtype=np.float32)
rel_rec = torch.FloatTensor(rel_rec)    # 将 NumPy 数组转换为 PyTorch 张量。
...
# 如果指定了加载模型的文件夹，则加载已经训练好的编码器和解码器模型
if args.load_folder:
...
optimizer = optim.Adam(list(encoder.parameters()) + list(decoder.parameters()),    # 创建 Adam 优化器，用于更新模型参数。
                       lr=args.lr)
scheduler = lr_scheduler.StepLR(optimizer, step_size=args.lr_decay,    # 创建学习率调度器，用于在训练过程中逐渐降低学习率。
                                gamma=args.gamma)
...
# 如果使用 GPU，则将编码器、解码器、接收关系张量、发送关系张量、三角形矩阵索引等移动到 GPU 上。
if args.cuda:
    encoder.cuda()
    decoder.cuda()
...
def train(epoch, best_val_loss):
    # 该函数用于执行模型训练，包括计算损失函数、反向传播、更新模型参数等操作。
    t = time.time()
    nll_train = []
    acc_train = []
    kl_train = []
